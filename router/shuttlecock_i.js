...
router.get('/', (req, res) => {
  ...
  switch (datekind) {
    case 'semester': // 학기중
      if (daykind == 'week') { // 주중
        var jsonPath = path.join(__dirname, '..', 'timetable', 'semester', 'week', 'Shuttlecock_I_week.json')
      } else if (daykind == 'weekend') { // 주말
        var jsonPath = path.join(__dirname, '..', 'timetable', 'semester', 'weekend', 'Shuttlecock_I_weekend.json')
      }
      break
    case 'vacation': // 방학중
      if (daykind == 'week') { // 주중
        var jsonPath = path.join(__dirname, '..', 'timetable', 'vacation', 'week', 'Shuttlecock_I_week.json')
      } else if (daykind == 'weekend') { // 주말
    ...
    case 'vacation_session': // 계절학기중
      if (daykind == 'week') { // 주중
        ...
        var jsonPath = path.join(__dirname, '..', 'timetable', 'vacation_session', 'weekend', 'Shuttlecock_I_weekend.json')
      }
      break
    default:
      break
  ...
  if (daykind == 'week') { //주중일 경우
    ...
    return res.json(jsondata.shuttlecock_i_weekend)
  }
})
